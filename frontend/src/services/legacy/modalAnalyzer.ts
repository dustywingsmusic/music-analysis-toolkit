 ...
 *
 * Status: LEGACY - Superseded by enhancedModalAnalyzer.ts which has stronger rules
 * and better confidence scoring. This version is kept for historical reference.
 *
 * Key Features:
...
} from '../experimental/musicTheoryEngine';
...
export class ModalAnalyzer {
  ...
  private getRomanNumeral(intervalFromTonic: number, isMinor: boolean, quality: any): string {
    ...
    const chromaticRomanNumerals: Record<number, string> = {
      0: isMinor ? 'i' : 'I',      // Tonic (for suspended variants)
      1: '♭II',   // Neapolitan
      3: isMinor ? '♭III' : '♭III', // Modal interchange
      4: isMinor ? '#iv°' : '#IV°',   // Raised 4th diminished (C#° in A minor)
