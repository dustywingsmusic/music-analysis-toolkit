...
class ComprehensiveTestGenerator {
    ...
    generateAllTestCases() {
        ...
        this.generateLongformSequences();
    ...
    /**
    * 6. LONGFORM SEQUENCES
    * Generate multi-segment progressions with expected segmentations.
    */
    generateLongformSequences() {
        console.log(' Generating longform sequences...');

        // Helper: choose spelling preference from parent key
        const preferFromParent = (parentKey) => {
            const pk = parentKey || '';
            const pkRoot = pk ? parseRootName(pk) : '';
            if (pkRoot && FLAT_MAJOR_ROOTS.has(pkRoot)) return 'flats';
            if (pk.includes('#')) return 'sharps';
            return 'auto';
        };

        // Helper: degree from an arbitrary root using parent key spelling preference
        const degFrom = (base, semitones, parentKey) => {
            const prefer = preferFromParent(parentKey);
            return this.getNoteAtInterval(base, semitones, { prefer });
        };

        // --- LF1: Mixolydian vamp -> Functional ii–V–I (C Mixolydian in F, then C Ionian in C) ---
        // Seg1 (0..3): C Bb F C (I–bVII–IV–I)
        // Seg2 (4..8): Dm G C F G (ii–V–I–IV–V)
        {
            const seg1Parent = 'F major';
            const seg2Parent = 'C major';
            const C = 'C';
            const Bb = degFrom(C, 10, seg1Parent);
            const F = degFrom(C, 5, seg1Parent);
            const Dm = `${degFrom('D', 0, seg2Parent)}m`;
            const G = degFrom('G', 0, seg2Parent);
            const chords = [C, Bb, F, C, Dm, G, 'C', 'F', G];
            ...
            this.addTestCase({
                ...
                category: 'longform',
                expectedModal: false,
                description: 'LF1: C Mixolydian vamp then functional ii–V–I',
                expectedSegments: [
                    { start: 0, end: 3, parentKey: seg1Parent, mode: 'C Mixolydian' },
                    { start: 4, end: 8, parentKey: seg2Parent, mode: 'C Ionian' }
                ]
            });
        }

        // --- LF2: Dorian i–IV–i -> Aeolian i–bVI–bVII–i (A Dorian in G, A Aeolian in C) ---
        {
            const dorParent = 'G major';
            const aeolParent = 'C major';
            const A = 'A';
            const D = degFrom(A, 5, dorParent);
            const F = degFrom(A, 8, aeolParent);
            const G = degFrom(A, 10, aeolParent);
            const chords = [`${A}m`, D, `${A}m`, `${A}m`, F, G, `${A}m`];
            ...
            this.addTestCase({
                ...
                category: 'longform',
                expectedModal: false,
                description: 'LF2: A Dorian vamp then A Aeolian cadence',
                expectedSegments: [
                    { start: 0, end: 2, parentKey: dorParent, mode: 'A Dorian' },
                    { start: 3, end: 6, parentKey: aeolParent, mode: 'A Aeolian' }
                ]
            });
        }

        // --- LF3: Lydian strict (no V) -> Lydian with V (F Lydian in C) ---
        {
            const parent = 'C major';
            const F = 'F';
            const G = degFrom(F, 2, parent); // II
            const C = degFrom(F, 7, parent); // V of F
            const chords = [F, G, F, G, F, G, C, F];
            ...
            this.addTestCase({
                ...
                category: 'longform',
                expectedModal: false,
                description: 'LF3: F Lydian strict I–II–I–II then I–II–V–I',
                expectedSegments: [
                    { start: 0, end: 3, parentKey: parent, mode: 'F Lydian' }, // strict long_nov flavor
                    { start: 4, end: 7, parentKey: parent, mode: 'F Lydian' } // permissive with V
                ]
            });
        }

        // --- LF4: Ionian with tonicization V/ii → ii → V → I (C major) ---
        {
            const parent = 'C major';
            const F = degFrom('C', 5, parent);
            const A7 = `${degFrom('A', 0, parent)}7`; // V/ii
            const Dm = `${degFrom('D', 0, parent)}m`;
            const G = degFrom('G', 0, parent);
            const C = 'C';
            const chords = [C, F, A7, Dm, G, C];
            ...
            this.addTestCase({
                ...
                category: 'longform',
                expectedModal: false,
                description: 'LF4: C Ionian with brief tonicization (V/ii → ii → V → I)',
                expectedSegments: [
                    { start: 0, end: 1, parentKey: parent, mode: 'C Ionian' },
                    { start: 2, end: 5, parentKey: parent, mode: 'C Ionian', functionalDetail: 'V/ii → ii → V → I' }
                ]
            });
        }
    }
            ...
            return JSON.stringify({
                metadata: {
                    ...
                    categories: {
                        ...
                        longform: this.testCases.filter(t => t.category === 'longform').length
                    }
                },
...
function exportComprehensiveTestCases() {
    ...
    // Save to file
    fs.writeFileSync('./comprehensive-modal-test-cases_w_long_cjs.json', jsonOutput);
    console.log('💾 Test cases saved to comprehensive-modal-test-cases_w_long_cjs.json');
